...
export default class EtherscanService {
  ...
  async isAuthenticated(networkName: string) {
    ...
    if (etherscanAPIKey && etherscanAPIKey.length > 0) {
      const auth = await axios({
        method: 'GET',
        url: `${NETWORK_EXPLORERS[networkName]}/api?module=account&action=balance&address=${account}&tag=latest&apikey=${etherscanAPIKey}`,
      });
  ...
  async getContractABI(address: string, networkName: string) {
    ...
    return axios({
      method: 'GET',
      url: `${NETWORK_EXPLORERS[networkName]}/api?module=contract&action=getabi&address=${address}&apikey=${etherscanAPIKey}`,
    });
  }
  ...
  async getContractSource(address: string, networkName: string) {
    ...
    return axios({
      method: 'GET',
      url: `${NETWORK_EXPLORERS[networkName]}/api?module=contract&action=getsourcecode&address=${address}&apikey=${etherscanAPIKey}`,
    });
  }
}
