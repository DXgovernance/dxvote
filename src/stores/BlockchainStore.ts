...
export default class BlockchainStore {
  ...
  // }
  // 
  // async updateProposalData(cacheToUpdate: DaoNetworkCache, schemeAddress: string, proposalId: string): DaoNetworkCache {
  //   const { configStore, providerStore } = this.rootStore;
  //   const { library } = providerStore.getActiveWeb3React();
  // 
  ...
  // 
  //     if (!cacheToUpdate.proposals[proposalId]) {
  //       const schemeContract = providerStore.getContract(
  //         providerStore.getActiveWeb3React(),
  //         ContractType.WalletScheme,
  //         schemeAddress
  //       )
  //       const creationEvent = (await schemeContract.getPastEvents('NewCallProposal', {
  //         fromBlock: cacheToUpdate.blockNumber, filter: { _proposalId: proposalId }
  //       }))[0]
  //       cacheToUpdate.schemes[schemeAddress].newProposalEvents.push({
  //         proposalId: proposalId,
  //         event: creationEvent.event,
  //         signature: creationEvent.signature,
  //         address: creationEvent.address,
  //         txSender: (await library.eth.getTransaction(creationEvent.transactionHash)).from,
  //         tx: creationEvent.transactionHash,
  //         block: creationEvent.blockNumber,
  ...
  //       console.log(controllerEvent.returnValues)
  //       const schemeContract = providerStore.getContract(
  //         providerStore.getActiveWeb3React(),
  //         ContractType.WalletScheme,
  //         controllerEvent.returnValues._scheme
  ...
  //   await Promise.all(votingMachineEvents.map(async (votingMachineEvent) => {
  //     const txSender = (await library.eth.getTransaction(votingMachineEvent.transactionHash)).from;
  // 
  ...
  //   await Promise.all(reputationEvents.map(async (reputationEvent) => {
  //     const txSender = (await library.eth.getTransaction(reputationEvent.transactionHash)).from;
  // 
  ...
  //         cache = await this.updateProposalData(cache, schemeAddress, cache.schemes[schemeAddress].proposalIds[i]);
  //     }
  //   }
  // 
  //   return cache;
