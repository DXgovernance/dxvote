...
async function main() {
  ...
  const dxdVotingMachine = await DXDVotingMachine.new(
    votingMachineToken.address,
    { gas: GAS_LIMIT }
  );
  const multicall = await Multicall.new();

  const permissionRegistry = await PermissionRegistry.new(accounts[0], 1, {
    gas: 1000000,
  });
  ...
  const notAllowedControllerFunctions = [
    ...
    controller.contract._jsonInterface.find(method => method.name == 'metaData')
      .signature,
  ];
  ...
  for (var i = 0; i < schemesConfiguration.length; i++) {
    ...
    await Promise.all(
      schemeConfiguration.permissions.map(async permission => {
        await permissionRegistry.setAdminPermission(
          permission.asset,
  ...
  await votingMachineToken.transfer(accounts[2], web3.utils.toWei('30'), {
    from: accounts[0],
  });
  ...
  let descriptionText =
    'Set 10 REP tokens to ' +
    accounts[0] +
    ', 20 REP tokens to ' +
    accounts[1] +
    ', and 70 REP tokens to ' +
    accounts[2];
  ...
  descriptionText =
    'Set required % of votes for boosted proposals to 1% to execute in Master Wallet.';
  const testProposal1 = (
    await schemes['MasterWalletScheme'].proposeCalls(
      [controller.address],
      [
        web3.eth.abi.encodeFunctionCall(generiCallABI, [
          ...
          web3.eth.abi.encodeFunctionCall(setBoostedVoteRequiredPercentageABI, [
            ...
            await controller.getSchemeParameters(
              schemes['MasterWalletScheme'].address,
              avatar.address
            ),
            1000,
  ...
  await dxdVotingMachine.vote(testProposal1, 1, 0, NULL_ADDRESS, {
    from: accounts[2],
  });

  // Create test proposal 2
  descriptionText =
    'Mint 20 REP, tranfer 15 ETH and 50 tokens to QuickWalletScheme.';
  const testProposal2 = (
    await schemes['MasterWalletScheme'].proposeCalls(
      [controller.address, controller.address, controller.address],
      [
        web3.eth.abi.encodeFunctionCall(mintReputationABI, [
          web3.utils.toWei('1'),
          accounts[1],
          avatar.address,
        ]),
        ...
        web3.eth.abi.encodeFunctionCall(generiCallABI, [
          votingMachineToken.address,
          web3.eth.abi.encodeFunctionCall(ERC20TransferABI, [
            schemes['QuickWalletScheme'].address,
            web3.utils.toWei('50'),
          ]),
          avatar.address,
  ...
  await dxdVotingMachine.vote(testProposal2, 1, 0, NULL_ADDRESS, {
    from: accounts[2],
  });
  ...
  await votingMachineToken.approve(
    dxdVotingMachine.address,
    await votingMachineToken.balanceOf(accounts[1]),
    { from: accounts[1] }
  );
  await dxdVotingMachine.stake(
    testProposal3,
    1,
    web3.utils.toWei('2.22222222').toString(),
    { from: accounts[1] }
  );
  await dxdVotingMachine.vote(
    testProposal3,
    1,
    web3.utils.toWei('5.77777777'),
    NULL_ADDRESS,
    {
      from: accounts[1],
    }
